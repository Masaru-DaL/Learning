...
nokori_count = 0                            # ２７）残りのマス数（初期値は０）
# マップに合わせて、青ブロックを配置する
for y in range(7):                          # 行数分の繰り返し
    for x in range(10):                     # 列数分の繰り返し
        if maze_map[y][x] == 1:             # 該当マップ箇所のデータの取得
            ...
            nokori_count += 1               # ２８）残りのマス数を１加算

my_x, my_y = 1, 1                           # 自キャラの初期位置を設定
# 自キャラの画像ファイルを読み込み
...
def main_proc():
    ...
    # 自キャラの位置を移動させる
    global my_x, my_y, nokori_count         # ２９）残りのマス数をglobal指定
