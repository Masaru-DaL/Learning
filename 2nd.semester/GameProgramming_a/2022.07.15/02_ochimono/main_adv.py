...
def add_neko():
    ...
    for x in range(8):
        # ねこの種類を難易度＋２にする
        neko_field[0][x] = random.randint(0, difficulty+2)
...
def game_main():
    ...
    # スタート待ち
        ...
        # クリックされたら
        if ms_click == True:
            # 難易度がクリックされたら、難易度（２～４）を設定
            if 168 < ms_x < 456 and 384 < ms_y < 456:
                difficulty = 2
            if 168 < ms_x < 456 and 528 < ms_y < 600:
                difficulty = 3
            if 168 < ms_x < 456 and 672 < ms_y < 744:
                difficulty = 4

            # 難易度が設定された場合のみ以下処理を実施
            if difficulty > 0:
                # ねこフィールドをクリア
                for y in range(10):
                    for x in range(8):
    ...
    # 消去処理
        ...
        # スコアの計算に、難易度を加味する
        # スコアに、「消した数✕連鎖数✕難易度✕5」を加算
        score += sw * chain * difficulty * 5
        ...
        if sw > 0:
            ...
            if check_gameover() == False:
                # 次のねこセットを難易度＋２にする
                next_neko = random.randint(1, difficulty+2)
                step = 5            # マウス入力へ
            else:
                step = 6            # ゲームオーバーへ
    ...
    cvs.delete("INFO")
    # スコアを描画
    s_font = ("Arial", 32, "bold")
    cvs.create_text(160, 60, text=f"SCORE {score}", fill="blue",
                    font=s_font, tag="INFO")
    # ハイスコアを描画
    cvs.create_text(450, 60, text=f"HI-SCO {hi_score}", fill="orange",
                    font=s_font, tag="INFO")
