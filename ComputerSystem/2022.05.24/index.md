...
- ブリッジ(bridge)
  - OSIの第2層でセグメントを接続する中継装置
  - 接続されている機器のMACアドレスを自動的に学習し、送信データに含まれる宛先MACアドレスの機器が接続されているポートにだけデータを中継します

- ルータ(router)
  - 異なるネットワーク間をOSIの第3層で接続するための装置で、宛先アドレスを用いてパケットの中継と経路制御を行うルーティング機能を持つ

- ゲートウェイ(gateway)
  - トランスポート層以上が異なるネットワーク相互間を、全ての層でプロトコル変換を行うことにより接続する装置
...
## 出典：令和4年春期　問79 アルゴリズムとプログラミング　(全18問中2問目)
https://www.itpassportsiken.com/bunya.php?m=14&s=2&no=2
...
# データの伝送
## 出典：令和2年秋期　問95 ネットワーク方式　(全65問中10問目)
https://www.itpassportsiken.com/bunya.php?m=22&s=1&no=10

伝送速度が20Mbps(ビット／秒)，伝送効率が80%である通信回線において，1Gバイトのデータを伝送するのに掛かる時間は何秒か。ここで，1Gバイト＝103Mバイトとする。
(エ)500 秒

> 20Mbps -> 80%
> 20 / 80% -> 16 Mbps // バイトに直さないといけない
> 16 / 8 -> 1秒間に2MB
> 1GBを2MBで割る -> 1000 / 2 = 500秒

## 出典：平成30年春期　問31 ネットワーク方式　(全37問中5問目)
https://www.fe-siken.com/bunya.php?m=10&s=1&no=5

10Mバイトのデータを100,000ビット／秒の回線を使って転送するとき，転送時間は何秒か。ここで，回線の伝送効率を50%とし，1Mバイト＝106バイトとする。
(エ) 1600

> 100000 / 8 -> 12500 B/秒 // バイトに直す
> 伝送効率50% 12500 / 2 -> 6250B
> 10000000 / 6250 -> 1600秒かかる
...
- ファイル転送ほか
  - HTTP
    - HTMLファイルを転送し、WWWを実現するためのプロトコル
  - FTP
    - ファイルを転送するためのプロトコル
- 管理用
  - NTP
    - ネットワークを介して、コンピュータの時計を合わせるプロトコル

- ポート番号
  - ポート番号によって識別されるものは、コンピュータ上で動作している通信アプリケーション

# アルゴリズム
## 出典：サンプル問題1　問42 アルゴリズムとプログラミング　(全18問中18問目)
...
6. 入替えが発生していたときは，(1) から繰り返す。

1.  51432
2. 14325
3. 13245
4.  12345 (整列は終わっているが入れ替えが発生しているので再度繰り返す)
5.  12345 (一度も入れ替えがなかったので終了)
(答) 4回
